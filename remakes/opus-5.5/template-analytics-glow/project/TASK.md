# Task: Growing Analytics Capsules (`template-analytics-glow`)

Build this motion component from scratch in this Remotion project.

## Spec

960x540, 60 fps, 180 frames (3.00 s). Grow softly rounded data columns in a small white analytics card and emphasize one column with a warm glow. Keep white-on-white depth, restrained shadows, quiet explanatory text and a compact final result.

## Project

- `src/index.tsx` registers composition `template-analytics-glow` at 960x540, 60 fps, 180 frames. Keep those exactly; put the implementation in `src/` (any number of files).
...
- Final: `node scripts/render.mjs` renders `out/template-analytics-glow.mp4`.
