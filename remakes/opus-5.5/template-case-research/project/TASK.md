# Task: Evidence-Backed Answer (`template-case-research`)

Build this motion component from scratch in this Remotion project.

## Spec

960x540, 30 fps, 252 frames (8.40 s). Expand a query surface into an answer with evidence. Carry the same corner markers into a layered photographic proof scene, then converge them into a brand ending. Preserve warm light, lens softness and continuous marker movement.

## Project

- `src/index.tsx` registers composition `template-case-research` at 960x540, 30 fps, 252 frames. Keep those exactly; put the implementation in `src/` (any number of files).
...
- Final: `node scripts/render.mjs` renders `out/template-case-research.mp4`.
