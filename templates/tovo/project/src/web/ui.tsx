...
export const uiInk=(b:Brand)=>b.template==='NUMO'?'#163C30':b.template==='FORMO'?b.background:b.ink;
...
export const Button:React.FC<{b:Brand;x:number;y:number;w?:number;h?:number;icon?:IconName;children?:React.ReactNode;quiet?:boolean;pressed?:boolean;style?:React.CSSProperties}>=({b,x,y,w=260,h=80,icon='arrow',children,quiet=false,pressed=false,style})=><Box x={x} y={y} w={w} h={h} style={{background:quiet?'#FFFFFF':uiAccent(b),color:quiet?uiInk(b):'#FFF8F2',border:quiet?'1.5px solid #28202525':'1.5px solid transparent',borderRadius:12,display:'flex',alignItems:'center',justifyContent:'center',gap:18,fontSize:48,fontWeight:560,letterSpacing:'-.04em',transform:pressed?'scale(.96)':'none',boxShadow:quiet?'0 2px 3px #20102004':'0 3px 6px #20102014',...style}}>{children}<Icon name={icon} size={38}/></Box>;
export const Avatar:React.FC<{b:Brand;size?:number;active?:boolean}>=({b,size=54,active=false})=><div style={{width:size,height:size,borderRadius:99,background:active?b.accent:b.paper,color:b.ink,display:'grid',placeItems:'center',border:'2px solid #ffffff',flexShrink:0}}><Icon name="user" size={size*.56}/></div>;
export const AppFrame:React.FC<{b:Brand;f:number;title:string;active?:IconName;children:React.ReactNode;action?:React.ReactNode;camera?:number}>=({b,f,title,active='grid',children,action,camera=1})=>{
 const p=out(f,14);return <Stage brand={b} grain={false}><Box x={52} y={44+(1-p)*24} w={1176} h={632} style={{background:'#F5F4F3',border:`1px solid ${b.ink}24`,borderRadius:18,boxShadow:`0 28px 65px ${b.ink}28,0 2px 4px ${b.ink}18`,overflow:'hidden',scale:camera+(1-p)*.025,transformOrigin:'center center',color:uiInk(b)}}>
 <Box w={1176} h={74} style={{background:'#FFFEFD',borderBottom:'1px solid #29252b1b',display:'flex',alignItems:'center',padding:'0 23px',gap:10}}>{[0,1,2].map(i=><div key={i} style={{width:11,height:11,borderRadius:20,background:['#E3AAA6','#E4C797','#B0C6AE'][i]}}/>)}<Box x={99} y={17}><Mark kind={b.template} size={36}/></Box><UText x={152} y={9} size={48} weight={590}>{title}</UText>{action||<Box x={1028} y={18} style={{display:'flex',gap:28,alignItems:'center'}}><Icon name="search"/><Icon name="more"/></Box>}</Box>
 <Box x={0} y={74} w={74} h={558} style={{borderRight:'1px solid #29252b14',background:'#FDFCFA',display:'flex',alignItems:'center',flexDirection:'column',gap:20,paddingTop:26}}>{(['grid',active==='grid'?'tasks':active,'calendar','file'] as IconName[]).map((name,i)=><div key={i} style={{width:52,height:52,borderRadius:11,display:'grid',placeItems:'center',background:i===1?`${uiAccent(b)}13`:'transparent',color:i===1?uiAccent(b):'#AAA5A2'}}><Icon name={name} size={29}/></div>)}<Box x={12} y={482}><Avatar b={b} size={48}/></Box></Box>
 <Box x={74} y={74} w={1102} h={558} style={{overflow:'hidden'}}>{children}</Box></Box></Stage>;
};
