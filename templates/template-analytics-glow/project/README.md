# Growing Analytics Capsules — editable source 1.1.0
...
Select `template-analytics-glow` in Studio. `npm run render` renders only this composition to `out/template-analytics-glow.mp4`: 960×540, 60 fps, 180 frames. This component is intentionally silent. `npm run render -- --preview` uses the gallery preview dimensions. `npm run stills` renders four inspection frames; stills alone are not full-video review.
...
Read `template.json`, `src/index.tsx`, its imported source files, `MOTION-NOTES.md`, and `ASSETS.md`. Find the Composition whose id is `template-analytics-glow` and edit its defaultProps and component implementation. Related compositions remain in the recovered batch source so shared helpers and geometry are preserved; the render script selects only this template. Do not substitute another composition.
