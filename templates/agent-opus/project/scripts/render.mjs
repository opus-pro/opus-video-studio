...
try {
  ...
  if (process.argv.includes("--stills-only") || selectedFrames) {
    ...
    for (const frame of selectedFrames ?? [
      18, 60, 96, 166, 215, 251, 277, 310, 329, 330, 401, 427, 463, 577, 599,
      600, 675, 714, 715, 778, 818, 848, 908, 956, 981, 1045, 1091, 1124, 1150,
      1180, 1210, 1223,
    ]) {
    ...
    await renderMedia({
      ...
      composition,
      puppeteerInstance: browser,
      outputLocation: path.join(out, frameRange ? `segment-${frameRange?.[0]}-${frameRange?.[1]}.mp4` : "product-launch.mp4"),
      frameRange,
      codec: "h264",
      crf: 16,
      pixelFormat: "yuv420p",
    ...
    console.log(frameRange ? `Saved dist/segment-${frameRange[0]}-${frameRange[1]}.mp4` : "Saved dist/product-launch.mp4");
  }
} finally {
