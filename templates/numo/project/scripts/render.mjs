...
try {
  ...
  if (process.argv.includes("--stills-only") || selectedFrames) {
    ...
    for (const frame of selectedFrames ?? [8,37,62,110,180,230,265,313,361,410,450,500,550,610,678,730,785,860]) {
    ...
    await renderMedia({
      ...
      composition,
      puppeteerInstance: browser,
      outputLocation: path.join(out, frameRange ? `segment-${frameRange?.[0]}-${frameRange?.[1]}.mp4` : "numo.mp4"),
      frameRange,
      codec: "h264",
      crf: 16,
      pixelFormat: "yuv420p",
    ...
    console.log(frameRange ? `Saved dist/segment-${frameRange[0]}-${frameRange[1]}.mp4` : "Saved dist/numo.mp4");
  }
} finally {
