...
export async function finalizeAudio() {
  ...
  const cfg = JSON.parse(
    fs.readFileSync(path.join(root, "config/brand.json")),
  );
  if (cfg.audio.mode === "mute") return;
  const timeline = JSON.parse(
    fs.readFileSync(path.join(root, "config/timeline.json")),
  );
  const movie = path.join(root, "dist/numo.mp4");
  const temp = path.join(root, "dist/numo-mux.mp4");
  const master = path.join(root, "public", cfg.audio[cfg.audio.mode]);
  const codec =
  ...
  await RenderInternals.callFf({
    ...
    args: [
      ...
      ...codec,
      "-t",
      String(timeline.durationInFrames / timeline.fps),
      "-map_metadata",
      "-1",
      "-movflags",
      "+faststart",
