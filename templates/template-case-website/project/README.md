# Live Website Restyle — editable source 1.1.0
...
Select `template-case-website` in Studio. `npm run render` renders only this composition to `out/template-case-website.mp4`: 960×540, 30 fps, 195 frames. This component is intentionally silent. `npm run render -- --preview` uses the gallery preview dimensions. `npm run stills` renders four inspection frames; stills alone are not full-video review.
