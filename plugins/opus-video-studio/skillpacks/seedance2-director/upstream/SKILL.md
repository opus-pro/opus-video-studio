...
Before generating any output, read the three required reference files. Read the style preset file
only when the user names or requests a Seedance Director preset style.
...
4. `references/style-presets.md` — Read only when the user names or requests a Seedance Director
   preset style, such as `premium-minimal` or `modern-saas`. These presets belong to this
   skillpack; do not treat them as a global style library.
...
| 15s | 10–15 | 3+ | ON |
| 16–30s (Seedance 2.5 only) | Pace to the brief; do not inflate shot count | 3+ | ON |

Model limits determine clip count, not this shot-density table. Seedance 2.0 supports up to 15
seconds in one clip; Seedance 2.5 supports up to 30 seconds in one clip. Keep a 30-second Seedance
2.5 sequence as one clip unless the user or an approved technical/creative constraint requires a
split.
